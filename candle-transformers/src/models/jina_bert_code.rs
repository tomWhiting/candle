//! # JinaBERT Code Model inference implementation
//!
//! Variant of JinaBERT for code embeddings (jina-embeddings-v2-base-code).
//! Uses a different FFN structure and Q/K layer norms compared to the base JinaBERT.
//!
//! See: [Jina Code Embeddings on HuggingFace](https://huggingface.co/jinaai/jina-embeddings-v2-base-code)
...
/// Configuration for JinaBERT Code model
...
/// Self-attention with Q/K layer norms (used by code model)
#[derive(Clone, Debug)]
...
impl BertSelfAttention {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        // Code model has Q/K layer norms
...
/// GEGLU MLP variant used by code model
/// Uses up_gated_layer + down_layer instead of gated_layers + wo
#[derive(Clone, Debug)]
struct BertGLUMLP {
...
impl BertGLUMLP {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        // Code model uses up_gated_layer (no bias) for combined gate+up projection
...
impl Module for BertGLUMLP {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        // GEGLU: gelu(gate) * up
        (gated.apply(&self.act) * non_gated)?.apply(&self.down_layer)
    }
}

/// Transformer layer with pre-norm architecture
/// Uses layer_norm_1 (pre-attention) and layer_norm_2 (pre-MLP)
#[derive(Clone, Debug)]
struct BertLayer {
...
impl BertLayer {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        // Code model has layer norms at transformer block level
    ...
    fn forward(&self, xs: &Tensor, bias: &Tensor) -> Result<Tensor> {
        ...
        // Pre-norm attention
        let residual = xs;
        let xs = self.layer_norm_1.forward(xs)?;
        let xs = self.attention.forward(&xs, bias)?;
        let xs = (xs + residual)?;

        // Pre-norm MLP
        let residual = &xs;
        let xs = self.layer_norm_2.forward(&xs)?;
...
/// JinaBERT Code Model
///
/// Variant of JinaBERT optimized for code embeddings.
/// Key differences from base JinaBERT:
/// - Q/K layer norms in attention
/// - Different MLP tensor naming (up_gated_layer/down_layer vs gated_layers/wo)
/// - Layer norms at block level instead of inside MLP
#[derive(Clone, Debug)]
